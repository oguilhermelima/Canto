...
export function WatchlistLaunchpadBlock({ title: _title }: { title: string }): React.JSX.Element | null {
  ...
  return (
    <section className="-mx-5 md:-mx-8 lg:mx-0">
      <div className="flex items-baseline justify-between px-4 md:px-8 lg:px-0">
        <div>
          <p className="text-sm font-medium text-muted-foreground">Fueled up and waiting for launch</p>
      ...
      <div className="group/carousel relative">
        ...
        <div ref={containerRef} onScroll={handleScroll} className="flex gap-4 overflow-x-auto overflow-y-visible px-5 pt-2 pb-4 scrollbar-none md:px-8 lg:px-5">
          {isLoading
            ...
            : data?.items.map((item) => (
                <MediaCard key={item.mediaId} id={item.mediaId} externalId={String(item.externalId)} provider={item.provider} type={item.mediaType as "movie" | "show"} title={item.title} posterPath={item.posterPath} year={item.year} href={mediaHref(item.provider, item.externalId, item.mediaType)} className="w-[150px] shrink-0 sm:w-[170px]" />
              ))}
        </div>
      </div>
    </section>
