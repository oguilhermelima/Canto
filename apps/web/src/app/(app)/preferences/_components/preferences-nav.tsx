...
function SidebarGroup({ group, activeSection }: { group: PreferencesSectionGroup; activeSection: string }): React.JSX.Element {
  return (
    <div>
      ...
      <div className="space-y-0.5">
        {group.items.map((item) => {
          ...
          return (
            <Link
              key={item.key}
              href={`/preferences?section=${item.key}`}
              scroll={false}
              replace
              className={cn(
...
export function PreferencesSidebar({ activeSection }: { activeSection: string }): React.JSX.Element {
  return (
    <nav className="hidden md:block">
      <div className="sticky top-24 space-y-6">
        {PREFERENCES_SECTIONS.map((group) => (
...
export function PreferencesMobileList(): React.JSX.Element {
  return (
