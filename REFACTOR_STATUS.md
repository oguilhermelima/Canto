...
- ✅ Wave Final (parcial): F1 (delete `infra/repositories.ts`), F2 (boundary rules em modo `warn`), F3 parcial (algumas regras promovidas), F5 (script `lint:strict`), F7 (CI workflow)
...
Após Wave 10, executar F3+F4 originais:

- Promover regras `warn → error` em `apps/web/eslint.config.js` (já feito parcialmente em base.js):
  - `no-unused-vars`, `no-unnecessary-condition`, `no-misused-promises`, `no-explicit-any`, `consistent-type-imports`, `import-x/consistent-type-specifier-style`
  - `react-hooks/exhaustive-deps`, `rules-of-hooks`, `refs`, `immutability`, `preserve-manual-memoization`
  - `@next/next/no-img-element`
- Sweep ~750 errors atualmente latentes em core + ~260 em web pós-promotion. Auto-fix primeiro (`pnpm lint --fix`), depois manual:
  - `no-floating-promises`: adicionar `void` ou `await`
  - `prefer-nullish-coalescing`: `||` → `??`
  - `no-non-null-assertion`: `!` → narrowing
  - `no-explicit-any`: `any` → `unknown` + narrow
  - `eqeqeq`: `==` → `===`
- Round-by-round até `pnpm lint:strict` retornar 0.

### Wave 12 — Build clean + final commit (~30-60 min)

- `pnpm -F @canto/web build` zero warnings (Next/Turbopack residuais)
- Final REFACTOR_STATUS.md update marcando tudo ✅
- Possível ajuste no CI workflow
