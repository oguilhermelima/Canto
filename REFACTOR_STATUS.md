...
### ✅ Status real (atualizado 2026-04-30)

**Concluídas e merged em `main`:**
- ✅ Wave 0A: ESLint em todos os packages
- ✅ Wave 0B: react-hooks bugs em apps/web
- ✅ Wave 1: notifications
- ✅ Wave 2: user (+ adoção da convenção `@canto/<pkg>/...`)
- ✅ Wave 3: lists
- ✅ Wave 4: recommendations
- ✅ Wave 5a: media-servers infrastructure
- ✅ Wave 5b: media-servers use-case adoption
- ✅ Wave 6: trakt context
- ✅ Wave 6 finalize: TraktApiPort + cross-context wireup
- ✅ Wave 7: user-media
- ✅ Wave 8: torrents + file-organization
- ✅ Wave 9A: media core (media/season/episode)
- ✅ Wave 9B: media localization + aspect-state + persist
- ✅ Wave 9C: media extras + content-enrichment shims
- ✅ Wave 9C2: cleanup deferred items (overlays, fetch-logos, library JOINs, blocklist callers)
- ✅ Wave Final (lockdown): F1-F3, F5, F7, F8 — boundary rules instaladas (modo `warn` até Wave 10), regras de qualidade promovidas a `error` no possível, `lint:strict` script + CI workflow com staging plano. F4 (sweep) e F6 (build clean) deferidos para Waves 11-12 por escopo (~700 violations latentes).

**Pendentes:**

### Wave 10 — Eliminar 263 boundary leaks em domain/ (~6-9h)

A regra de boundary do Wave Final ficou em `warn` por causa de 263 violations descobertas. Cada uma é um vazamento real onde domain importa direto de infra/platform/drizzle-orm em vez de usar port. Distribuição:

- 18 em `domain/media/use-cases`
- 16 em `domain/torrents/use-cases`
- 14 em `domain/user-media/use-cases`
- 10 em `domain/recommendations/use-cases`
- 10 em `domain/media-servers/use-cases`
- 8 em `domain/trakt/use-cases`
- Restante distribuído em file-organization, content-enrichment, lists, sync, services, rules

Por categoria:
- 142 imports de helpers `drizzle-orm` runtime (eq, and, sql, inArray) — cada query inline em domain precisa virar método de port
- 21 imports diretos `infra/media/media-repository`
- 16 `platform/logger/log-error` — precisa LoggerPort
- 11 `platform/queue/bullmq-dispatcher` — JobDispatcherPort já existe mas é bypassada
- 11 `media-localization-repository.adapter` — ports já existem, só threading deps
- 9 `media-localized-repository`, 8 cada em lists/list-repo, file-organization/folder-repo, torrents/download-repo

**Estratégia**: split em sub-waves por contexto (Wave 10a/b/c/...). Cada sub-wave threada deps através das use cases que vazam, full-port-ifica chamadas. Promove boundary rule pra `error` ao final.

### Wave 11 — Lint hardening sweep (~3-4h)

Após Wave 10, completar F3+F4 originais. Estado atual pós Wave Final:

**Já promovidas a `error` em `tooling/eslint/base.js` e `apps/web/eslint.config.js`:**
- `no-unused-vars`, `no-unnecessary-condition`, `no-misused-promises`, `consistent-type-imports`, `import-x/consistent-type-specifier-style`
- `no-floating-promises`, `await-thenable`, `no-unnecessary-type-assertion`
- `no-explicit-any` (em web)
- `react-hooks/exhaustive-deps`, `rules-of-hooks`, `refs`, `immutability`, `preserve-manual-memoization`
- `@next/next/no-img-element`

**Permanecem em `warn` até o sweep ser feito (em `tooling/eslint/base.js`):**
- `@typescript-eslint/no-non-null-assertion` (~224 violations)
- `@typescript-eslint/prefer-nullish-coalescing` (~76 violations)
- `eqeqeq` (~128 violations)

Sweep dos errors restantes (~370 contando os pre-existing legacy):
- `no-unnecessary-condition` (~197): mais comum; legacy code com checks defensivos
- `no-unused-vars` (~20)
- `no-img-element` (~10): converter para `next/image`
- `no-explicit-any` (~7): `any` → `unknown` + narrow
- `consistent-type-imports` (~6): autofix
- `no-empty-object-type` (~1)

Auto-fix primeiro (`pnpm lint:fix`), depois manual:
- `no-floating-promises`: adicionar `void` ou `await`
- `prefer-nullish-coalescing`: `||` → `??` (revisar 0/"" semântica)
- `no-non-null-assertion`: `!` → narrowing
- `eqeqeq`: `==` → `===` (cuidado com `== null`)

Round-by-round até `pnpm lint:strict` retornar 0. Promove rules acima de `warn` → `error`.

### Wave 12 — Build clean + CI strict (~30-60 min)

- `pnpm -F @canto/web build` zero warnings (Next/Turbopack residuais)
- Trocar passo `Lint` no CI workflow `.github/workflows/ci.yml` por `Lint (strict)` — `run: pnpm lint:strict` (`--max-warnings=0`)
- Final REFACTOR_STATUS.md update marcando tudo ✅

---

### Total

| Bloco | Estimativa | Status |
|---|---|---|
| Wave 0 (bootstrap) | 3-4h | ✅ |
| Waves 1-9 (contextos) | 7-9h agregado | ✅ |
| Wave 9C2 (cleanup) | 1.5h | ✅ |
| Wave Final (lockdown) | 2h | ✅ |
| Wave 10 (boundary leaks) | 6-9h | ⏳ |
| Wave 11 (lint sweep) | 3-4h | ⏳ |
| Wave 12 (build + strict CI) | 30-60 min | ⏳ |
| **Total agregado** | **~22-28h** | |
...
### Wave Final (lockdown) — registro do que foi feito (2026-04-30)

- **F1 — Delete `infra/repositories.ts` aggregate barrel**: 21 consumers migrados para paths específicos (`@canto/core/infra/<ctx>/<repo>`), barrel deletado. Typecheck 10/10 ✅, tests 144/144 ✅.
- **F2 — Boundary rules em `packages/core/eslint.config.js`**: `no-restricted-imports` configurado para `domain/**` bloqueando `@canto/core/infra/*`, `@canto/core/platform/*`, runtime helpers de `drizzle-orm`, `bullmq`, `ioredis`, `@trpc/server`, `next`, `react`. Modo `warn` (será promovido a `error` na Wave 10). Fixture em `packages/core/src/__eslint_fixtures__/should-fail.ts` documenta o efeito da regra.
- **F3 — Rules promovidas em `tooling/eslint/base.js` e `apps/web/eslint.config.js`**: vide tabela acima na Wave 11.
- **F5 — `lint:strict` script** em root `package.json`: `"turbo run lint -- --max-warnings=0"` (não roda hoje, será o gate da Wave 12).
- **F7 — CI workflow** em `.github/workflows/ci.yml`: typecheck + tests + lint (não-strict). Comment indica plano de promover para `lint:strict` na Wave 12.
- **F8 — REFACTOR_STATUS update**: este documento.

Pequenos cleanups oportunistas neste round (ui/db/codemod/worker zero errors):
- `packages/ui`: convertido `interface ... extends X {}` para `type = X`, `==` defensivo com tipos `T | undefined` substituído.
- `packages/db`: `seed-watch-providers` usa `void seed()` + `console.warn`; `settings.ts` decrypt narrowing reescrito sem `!`; `getDerivedKey` usa `Object.hasOwn` para narrowing safe.
- `scripts/codemod`: removidos imports não-usados; `verify` accumulators marcados `_`.
- `apps/worker`: `process.on(SIGTERM/SIGINT)` envolto em `void shutdown()`; `seed-management` consolidado para checks `!== null`; `reverse-sync` filtro simplificado; `console.log` → `console.warn` em hot paths.
