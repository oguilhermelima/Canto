    ...
    .mutation(({ ctx, input }) => {
      ...
      if (!input.mediaId && !input.dryRun) {
        void (async () => {
          ...
          await ensureMediaMany(ctx.db, { ...makePersistDeps(ctx.db), dispatcher: jobDispatcher }, filter, spec);
        })().catch((err) => {
      ...
      return ensureMediaMany(ctx.db, { ...makePersistDeps(ctx.db), dispatcher: jobDispatcher }, filter, spec, { dryRun: input.dryRun });
    }),
