...
export const SETTINGS_REGISTRY = {
  ...
  // ── Cadence: enrichment scheduling knobs ─────────────────────────────────
  "cadence.emptyOutcomeCooldownDays": def({
    group: "cadence",
    label: "Empty Outcome Cooldown (days)",
    help: "How long to wait before re-trying an aspect that came back empty",
    schema: z.coerce.number().int().min(1).max(3650),
    default: 90,
    secret: false,
    inputType: "number",
  }),
  "cadence.http4xxMaxAttempts": def({
    group: "cadence",
    label: "HTTP 4xx Max Attempts",
    help: "Park the row permanently after this many consecutive 4xx failures",
    schema: z.coerce.number().int().min(1).max(20),
    default: 3,
    secret: false,
    inputType: "number",
  }),
  "cadence.http5xxBaseBackoffMin": def({
    group: "cadence",
    label: "HTTP 5xx Base Backoff (minutes)",
    help: "Base for the exponential backoff applied to transient provider errors",
    schema: z.coerce.number().int().min(1).max(1440),
    default: 5,
    secret: false,
    inputType: "number",
  }),
  "cadence.movieFreshWindowMonths": def({
    group: "cadence",
    label: "Movie Fresh Window (months)",
    help: "Movies released within this window are considered fresh and refreshed often",
    schema: z.coerce.number().int().min(0).max(120),
    default: 6,
    secret: false,
    inputType: "number",
  }),
  "cadence.movieFreshFreqDays": def({
    group: "cadence",
    label: "Fresh Movie Refresh (days)",
    help: "Refresh frequency for movies inside the fresh window",
    schema: z.coerce.number().int().min(1).max(3650),
    default: 30,
    secret: false,
    inputType: "number",
  }),
  "cadence.movieAgedFreqDays": def({
    group: "cadence",
    label: "Aged Movie Refresh (days)",
    help: "Refresh frequency for movies older than the fresh window",
    schema: z.coerce.number().int().min(1).max(3650),
    default: 365,
    secret: false,
    inputType: "number",
  }),
  "cadence.showFallbackFreqDays": def({
    group: "cadence",
    label: "Show Fallback Refresh (days)",
    help: "Generic fallback frequency used when no better signal is available",
    schema: z.coerce.number().int().min(1).max(3650),
    default: 7,
    secret: false,
    inputType: "number",
